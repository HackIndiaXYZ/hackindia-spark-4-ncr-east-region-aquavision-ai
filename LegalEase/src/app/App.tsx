...
export default function App() {
  ...
  const handleExport = () => {
    ...
    try {
      setIsExporting(true);
      const riskLabel = (level: string) => level === "high" ? "🔴 HIGH" : level === "medium" ? "🟡 MEDIUM" : "🟢 LOW";
      const lines = [
        "═══════════════════════════════════════════",
        "         LEGALEASE — CONTRACT RISK REPORT",
        "═══════════════════════════════════════════",
        "",
        ...
        "KEY CONSEQUENCES",
        "───────────────────────────────────────────",
        ...analysis.consequences.map((c, i) => `  ${i + 1}. ${c}`),
        "",
        "───────────────────────────────────────────",
        ...
        ...analysis.risks.map((r, i) => [
          ``,
          `  ${i + 1}. [${riskLabel(r.level)}] ${r.title}`,
          `     ${r.description}`,
          `     ⚠ Consequence: ${r.consequence}`,
        ].join("\n")),
        "",
        "═══════════════════════════════════════════",
        "Generated by LegalEase AI — for reference only.",
        "═══════════════════════════════════════════",
      ];
      ...
      a.href = url;
      a.download = `LegalEase_Report_${(selectedFile?.name || "contract").replace(/\.[^.]+$/, "")}.txt`;
