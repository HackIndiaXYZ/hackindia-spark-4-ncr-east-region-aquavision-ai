    ...
    <Accordion.Root type="multiple" defaultValue={defaultOpen} className="space-y-3">
      {categories.map((category, categoryIndex) => {
        ...
        return (
          ...
          >
            <Accordion.Header>
              <Accordion.Trigger className="group flex w-full items-center justify-between p-4 text-left transition-colors hover:bg-gray-50 focus:outline-none focus:ring-2 focus:ring-inset focus:ring-[#4f46e5] md:p-5">
                <div className="flex items-center gap-3">
                  <span className="text-xl">{icon}</span>
                  <span style={{ fontWeight: 600 }}>{category.title}</span>
                  <span className="flex h-6 w-6 items-center justify-center rounded-full bg-gray-100 text-xs text-muted-foreground" style={{ fontWeight: 600 }}>
            ...
            <Accordion.Content className="overflow-hidden border-t border-border data-[state=closed]:animate-accordion-up data-[state=open]:animate-accordion-down">
